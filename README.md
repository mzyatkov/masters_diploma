...
- **Источник данных в текущем прогоне:** по умолчанию `data.csv_path: null`, значит используется синтетический генератор из `src/data.py`.
- **Реальные данные поддерживаются:** достаточно указать путь `data.csv_path` в `configs/default.yaml` и привести колонки к ожидаемой схеме.
- **Uncertainty в основной модели:** в проекте используется именно **bootstrap-ансамбль CatBoost**, а не нестабильные version-dependent virtual ensembles.
- **Экономическая эффективность:** decision-aware метрики `ExpectedProfit` и `CVaR_alpha`, плюс `mean_profit/std_profit` для fixed policy.

## Ограничения текущей версии

- Экономическая модель однопериодная; инфляционный тренд и ценовые шоки в явном режиме не были частью исходной постановки.
- `remap` не моделируется как отдельное управленческое действие (есть превентивная/аварийная замена).
- Синтетика задает структурные зависимости признаков, но не калибрована на эмпирическую корреляционную матрицу конкретного реального парка.
